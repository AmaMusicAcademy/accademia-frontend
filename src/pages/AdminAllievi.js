      ...
      <div className="max-w-xl mx-auto px-4 pt-4 space-y-4">
        ...
        {loading ? (
          ...
          <>
            {tab === 'attivi' && (
              <>
                <p className="text-xs font-semibold text-gray-500 uppercase">Attivi ({filtra(attivi).length})</p>
            ...
            {tab === 'non_attivi' && (
              <>
                <p className="text-xs font-semibold text-gray-500 uppercase">Non attivi ({filtra(nonAttivi).length})</p>
            ...
            {tab === 'attesa' && (
              inAttesa.length === 0 ? (
                <div className="bg-white border border-dashed rounded-xl p-10 text-center">
                  ...
                  <p className="text-sm text-gray-400">Nessuna iscrizione in attesa.</p>
                </div>
              ) : (
                  ...
                  {inAttesa.map(a => (
                    <div key={a.id} className="bg-white border rounded-xl p-4 shadow-sm">
                      <div className="flex items-start justify-between mb-1">
                        <div>
                          <p className="font-semibold text-gray-900 text-sm">{a.nome} {a.cognome}</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-end justify-center">
          <div className="bg-white w-full max-w-lg rounded-t-2xl shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white flex items-center justify-between px-5 py-4 border-b z-10">
              <h2 className="text-base font-semibold text-gray-900">Nuovo allievo</h2>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-2">Assegna insegnante</label>
                <div className="border rounded-xl divide-y divide-gray-50 max-h-36 overflow-y-auto">
                  {insegnanti.filter(i => i.attivo !== false).map(i => (
                    <label key={i.id} className="flex items-center gap-3 px-3 py-2.5 cursor-pointer">
                      <div className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition-colors ${insSelezionati.includes(i.id) ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}
              ...
              <label className="flex items-center gap-3 cursor-pointer"
                ...
                <div className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 transition-colors ${qaPagata ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
