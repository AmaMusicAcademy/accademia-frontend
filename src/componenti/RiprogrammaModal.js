    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="text-base font-semibold text-gray-900">Riprogramma lezione</h2>
          <button onClick={onClose} className="text-gray-400 active:text-gray-700">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-5 py-4 space-y-4 max-h-[72vh] overflow-y-auto">
          ...
          {history.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5">
                {history.map((h, i) => (
                  <div key={i} className="bg-gray-50 border rounded-xl px-3 py-2 text-xs text-gray-600">
                    <span className="font-medium text-gray-800">#{i + 1}</span>
                    {' · '}{fmt(h.data_originale || h.data)}
                    {' '}{hhmm(h.ora_inizio)}–{hhmm(h.ora_fine)}
                    {h.aula ? ` · ${h.aula}` : ''}
                    {h.motivazione && <span className="block text-gray-400 italic mt-0.5">"{h.motivazione}"</span>}
                    {h.riprogrammata_il && (
          ...
          {/* nuova data/ora */}
          <div>
            <p className="text-xs font-medium text-gray-500 mb-2">Nuova data e orario</p>
            <div className="grid grid-cols-2 gap-2 mb-2">
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">Aula *</label>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs text-gray-400 mb-1">Inizio *</label>
