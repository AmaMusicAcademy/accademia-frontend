...
function KpiCard({ icon: Icon, color, label, value, onClick }) {
  const colors = {
    ...
    blue:    { bg: 'bg-ama-100',    icon: 'text-ama-500',    val: 'text-blue-700' },
    gray:    { bg: 'bg-n-50',    icon: 'text-n-600',    val: 'text-gray-700' },
  };
  ...
  return (
    ...
    >
      <div className={`flex items-center justify-between w-full`}>
        ...
        {onClick && <ChevronRight size={14} className="text-n-300" />}
      </div>
      <div>
    ...
    <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/40 px-0">
      <div className="bg-white w-full max-w-md rounded-t-2xl shadow-xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0">
          ...
          <button onClick={onClose} className="text-n-300"><X size={18} /></button>
        </div>
        <div className="overflow-y-auto flex-1 divide-y divide-gray-50">
          {lezioni.length === 0 && (
            <div className="p-6 text-center text-sm text-n-300">Nessuna voce.</div>
          )}
          {lezioni.map((l, i) => {
            ...
            return (
              <div key={l.id ?? i} className="px-4 py-3">
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <CalendarDays size={13} className="text-n-300 shrink-0" />
                ...
                {l.motivazione && (
                  <div className="flex items-start gap-1.5 mb-1.5">
                    <FileText size={12} className="text-n-300 mt-0.5 shrink-0" />
                    <p className="text-xs text-n-600 italic">"{l.motivazione}"</p>
                  </div>
                )}
                      ...
                      <div key={j} className="pl-3 border-l-2 border-amber-200 space-y-0.5">
                        <div className="flex items-start gap-1.5">
                          ...
                          <div className="text-xs text-n-600">
                            ...
                            {h.riprogrammata_il && (
                              <span className="text-n-300"> (il {fmtData(h.riprogrammata_il)})</span>
                            )}
                          </div>
                        </div>
                        {h.motivazione && (
                          <div className="flex items-start gap-1.5 pl-4">
                            <FileText size={11} className="text-n-300 mt-0.5 shrink-0" />
                            <p className="text-xs text-n-300 italic">"{h.motivazione}"</p>
                          </div>
                        )}
                      </div>
                    ))}
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="bg-white w-full max-w-md rounded-t-2xl shadow-xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b shrink-0">
          <button onClick={onClose} className="text-n-300 -ml-1">
            <ArrowLeft size={20} />
          </button>
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-semibold text-n-900 truncate">
              {allievo.nome} {allievo.cognome}
            </h2>
            {dataIscrizione && (
              <p className="text-xs text-n-300">Iscritto dal {fmtData(dataIscrizione)}</p>
            )}
          </div>
          {allievo.strumento && (
            <span className="text-xs bg-n-100 text-n-600 px-2 py-1 rounded-lg shrink-0">
              {allievo.strumento}
            </span>
          )}
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          ...
          {!loading && (
            <>
              ...
              {/* info extra */}
              {allievo.email && (
                <div className="bg-n-50 rounded-xl px-4 py-3">
                  <p className="text-xs text-n-300 mb-1">Email</p>
              ...
              {allievo.telefono && (
                <div className="bg-n-50 rounded-xl px-4 py-3">
                  <p className="text-xs text-n-300 mb-1">Telefono</p>
